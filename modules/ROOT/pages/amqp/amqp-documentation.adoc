...
** MINUTES
** HOURS
** DAYS  | +++Timeunit for the handshake timeout AMQP connection socket configuration. (From 1.5.0 onwards.)+++ |  +++MILLISECONDS+++ | {nbsp}
| handshakeTimeout | Integer | +++AMQP 0.9.1 timeout to set to the underlying AMQP connector. (From 1.5.0 onwards.)+++ |  +++default by AMQP client (10000 ms)+++ | {nbsp}
...
+++
Configuration for Socket in Frame Handler
+++
...
|===
| Name | Type | Description | Default Value | Required
| Keep Alive | Boolean | Keep alive to set to the underlying AMQP connector (From 1.5.0 onwards.) | false | {nbsp}
...
** MINUTES
** HOURS
** DAYS  | Timeunit for the SO_TIMEOUT AMQP connection socket configuration (From 1.5.0 onwards.) | MILLISECONDS | {nbsp}
| soTimeout | Integer | SO_TIMEOUT to set to the underlying AMQP connector (From 1.5.0 onwards.) | Default in user environment | {nbsp}
| receiveBufferSize | Integer | Receive buffer size to set to the underlying AMQP connector (From 1.5.0 onwards.) | Default in user environment | {nbsp}
| sendBufferSize | Integer | Send buffer size to set to the underlying AMQP connector (From 1.5.0 onwards.) | Default in user environment | {nbsp}
|===
